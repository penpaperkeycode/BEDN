function HardwareLayers=AutoConstructHWLayer_LastBatchExist(Net)
...
for i=1:size(Net.Layers,1)
    ...
    elseif ismethod(Net.Layers(i,1),'BinarizedTransposedConvolution2DLayer')
        HardwareLayers=[
            HardwareLayers;
    ...
    elseif ismethod(Net.Layers(i,1),'BinarizedFullyConnectedLayerFixer')
        if isempty(first_conv_finder)
            first_conv_finder=1;
        else
            first_conv_finder=2;
