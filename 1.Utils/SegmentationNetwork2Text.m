function SegmentationNetwork2Text(Net,Address)
...
LayerInputSize=Net.Layers(1, 1).InputSize(1:2);
...
for i=1:size(Net.Layers,1)
    ...
    if ismethod(Net.Layers(i,1),'Convolution2DLayer')
        if ~isempty((strfind(Net.Layers(i,1).Name,'Affine')))
            ...
            LayerInputSize_tmp1=1+floor((LayerInputSize(1)-Net.Layers(i,1).FilterSize(1)+2*Net.Layers(i,1).PaddingSize(1))/Net.Layers(i,1).Stride(1));
            LayerInputSize_tmp2=1+floor((LayerInputSize(2)-Net.Layers(i,1).FilterSize(2)+2*Net.Layers(i,1).PaddingSize(2))/Net.Layers(i,1).Stride(2));
            LayerInputSize=[LayerInputSize_tmp1,LayerInputSize_tmp2];
    ...
    elseif ismethod(Net.Layers(i,1),'MaxPooling2DLayer')
        ...
        MaxpoolLog=[ %Backup Input size of MaxPool layer
            MaxpoolLog
            LayerInputSize];
        
        %         LayerInputSize=1+floor(       (LayerInputSize-Net.Layers(i, 1).PoolSize(1))/Net.Layers(i, 1).Stride(1)    );
        LayerInputSize_tmp1=1+floor(       (LayerInputSize(1)-Net.Layers(i, 1).PoolSize(1))/Net.Layers(i, 1).Stride(1)    );
        LayerInputSize_tmp2=1+floor(       (LayerInputSize(2)-Net.Layers(i, 1).PoolSize(2))/Net.Layers(i, 1).Stride(2)    );
        LayerInputSize=[LayerInputSize_tmp1,LayerInputSize_tmp2];
    ...
    elseif ismethod(Net.Layers(i,1),'MaxUnpooling2DLayer')
        ...
        LayerInputSize=MaxpoolLog(end,:);
        MaxpoolLog=MaxpoolLog(1:end-1,:);
