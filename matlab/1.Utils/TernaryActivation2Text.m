function TernaryActivation2Text(SimNet,Address,I)
...
for i=1:size(SimNet.Layers,1)
    if TextoutputNameFlag==1
        ...
        txtName_flag=[Address,'\Activations\',num2str(i),'_',SimNet.Layers(i, 1).Name,'_flag','.txt'];
    else
        txtName=[Address,'\Activations\',SimNet.Layers(i, 1).Name,'.txt'];
        txtName_flag=[Address,'\Activations\',SimNet.Layers(i, 1).Name,'_flag','.txt'];
    end
    if ismethod(SimNet.Layers(i,1),'TernaryActivation')
