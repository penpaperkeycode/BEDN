classdef BConvolution2D < nnet.internal.cnn.layer.FunctionalLayer ...
    ...
    methods
        function this = BConvolution2D( ...
            ...
            % Initialize grouped-conv prediction cache if it might be needed
            this = clearPredictionCache(this);
        end
        ...
        function this = clearPredictionCache(this)
            ...
            if usingFilterGroups(this)
                % Every time a parameter changes we need a new cache handle to ensure that
                % multiple copies of the layer are not sharing the same cache.
    ...
    methods(Access = private)
        ...
        function this = setHostStrategy(this)
            ...
            if nnet.internal.cnnhost.useMKLDNN && noDilation
                this.ExecutionStrategy = BConvolution2DHostMkldnnStrategy();
            else
                this.ExecutionStrategy = BConvolution2DHostStridedConvStrategy();
            end
        end
        
        function this = setGPUStrategy(this)
            this.ExecutionStrategy = BConvolution2DGPUStrategy();
        end
    end
    ...
    methods(Access=protected)
        function this = setFunctionalStrategy(this)
            this.ExecutionStrategy = ...
                BConvolution2DFunctionalStrategy();
        end
    end
...
function noopCache = iCreateNullCache()
% Create a structure that can stand in for a CacheHandle but does not save
% any value and always reports empty.  This structure is not intended to be
% used, but provides a valid interface in case we get into a state where a
% cache is expected to exist but has not been created.
noopCache.isEmpty = true;
noopCache.fillCache = @(split) [];
end
